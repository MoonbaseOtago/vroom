...
class vroom(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","/home/paul/rv2/fvv")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for vroom start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' -l500000000 {2} | {3} > {1}'.format(self.isa, sig_file, "my.bin", "awk '{a[++k]=$$0}END{for(i=1;i<k;i++)print a[i]}'")
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.
          execute = '@cd {0}; {1}; {2}; {3};'.format(testentry['work_dir'], cmd, "riscv64-unknown-elf-objcopy  -O binary my.elf my.bin;riscv64-unknown-elf-objdump -d my.elf>my.code",  simcmd)
